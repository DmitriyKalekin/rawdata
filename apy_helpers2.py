...
def get_pool_history(pool):
    global ph
    if ph is None:
        ph = pd.read_sql(f"""SELECT
            pool, dt, tvlUsd as tvl, apy
        FROM pool_hist WHERE 1
        ORDER BY pool ASC, dt ASC""", con=dbcon, index_col=["pool", "dt"]) # ,
    dfp = ph.loc[pool].copy()
    return dfp
...
def apy_prediction_charts(df, dfp, corr_pools, today=["2022-09-13"]):
    cmap = mpl.cm.get_cmap('Paired')
    ...
    for i, pool in enumerate(df[df.index.isin(corr_pools)].index.values):
        ...
        ci = 1.96 * np.std(dpv["pred"].values)/np.sqrt(len(dpv["pred"].values))
        axs[i].fill_between(dpv["tvl"], (dpv["pred"]-ci), (dpv["pred"]+ci), color="#808080", alpha=.1)
...
def calc_hacker_risk(df, dfs):
    ...
    for i, row in dfs.iterrows():
        project = i[0]
        p = row["cnt"]
        df.loc[df.project==project, "num_pools"] = p
        df.loc[df.project==project, "hacker_resist"] = comb(int(t-p), b) / comb(t, b)
    return df
...
def calc_regression(df):
    ...
    for pool in pools:
        cond = df1.index == pool
        dfp = get_pool_history(pool)
        dfi = dfp.iloc[-FROM:].copy()  #.loc  [dfp.index[-FROM:], [col, des]]
        ...
        # Восстанавливаем предсказание по коэффициентам линейной регрессии
        dfi["pred"] = dfi["apy"] # d["intercept"] + sum([d[c] * dfi[c] for c in cols])
        ...
        df1.loc[cond, "apy.pred"] = df1.loc[cond, "apy"] # np.abs(
...
def simulate(df, amount, label):
    ...
    # dfi["emp1"] =  (1 - dfi["hacker_risk"]) * (dfi["amount"] + dfi["win"]) + dfi["hacker_risk"]*(-dfi["amount"])
    # dfi["emp2"] =  dfi["trust"] * (dfi["amount"] + dfi["win"]) + (1 - dfi["trust"])*(-dfi["amount"])

    # dfi["emp"] =  dfi["hacker_resist"] * dfi["trust"] * (dfi["amount"] + dfi["win"]/14*365)  + (1 - dfi["hacker_resist"] * dfi["trust"]) * (-dfi["amount"])  #     + dfi["hacker_risk"]*(-dfi["amount"])
    #dfi["emp"] =  dfi["hacker_resist"]  * (dfi["amount"] + dfi["win"]/14*365)  + (1 - dfi["hacker_resist"] ) * (-dfi["amount"])
    dfi["emp"] =  dfi["trust"] * (dfi["amount"] + dfi["win"]/14*365) + (1 - dfi["trust"] ) * (-dfi["amount"] )
    # dfi["emp2"] =  dfi["trust"] * (dfi["amount"] ) + (1 - dfi["trust"])*(-dfi["amount"])
...
def simulate_risk(df, amount, label, n):
    dfi = df.copy()
    dfi.sort_values(by=label, ascending=False, inplace=True)
    dfi["amount"] = 0
    dfi["amount_temp"] = np.minimum(dfi["tvl"]*0.1, amount)
    dfi["win_temp"] = dfi[label] * dfi["amount_temp"] / 365 / 100 * 14
    t = len(df.index)
    dfi["resist"] = dfi["hacker_resist"] * (1 - comb(t-dfi["num_pools"], n)/comb(t, n))
    # dfi["emp_temp"] = dfi["hacker_resist"] * dfi["trust"] * (dfi["amount_temp"] + dfi["win_temp"]/14*365)  + (1 - dfi["hacker_resist"] * dfi["trust"]) * (-dfi["amount_temp"])
    dfi["emp_temp"] = dfi["hacker_resist"] * (dfi["amount_temp"] + dfi["win_temp"]/14*365)  + (1 - dfi["hacker_resist"]) * (-dfi["amount_temp"])
    # dfi["win"] = 0
    # # dfi["E(win)"] = 0
    # dfi["asset"] = 0
    # dfi["der"] = 0
    ...
    # dfi["win"] = dfi["asset"] + dfi["der"]
    #
    dfi.sort_values(by="emp_temp", ascending=False, inplace=True)
    ...
    am = amount
    for i, row in dfi.iterrows():
        part = min(row["tvl"] * 0.1, am, amount/n)
        dfi.loc[i, "amount"] = part
        dfi.loc[i, "win"] = dfi.loc[i, label] * dfi.loc[i, "amount"] / 365 / 100 * 14
        # dfi.loc[i, "E(win)"] = part_risk * (-part) + (1-part_risk) * dfi.loc[i, "win"]
    ...
    # dfi["emp"] =  dfi["hacker_resist"] * dfi["trust"] * (dfi["amount"] + dfi["win"]/14*365)  + (1 - dfi["hacker_resist"] * dfi["trust"]) * (-dfi["amount"])  #     + dfi["hacker_risk"]*(-dfi["amount"])
    # dfi["emp"] =  dfi["resist"]  * (dfi["amount"] + dfi["win"]/14*365)  + (1 - dfi["resist"] ) * (-dfi["amount"])
    dfi["emp"] =  dfi["trust"] * (dfi["amount"] + dfi["win"]/14*365) + (1 - dfi["trust"] ) * (-dfi["amount"] )
    # dfi["emp1"] =  (1 - dfi["hacker_risk"]) * (dfi["amount"] ) + dfi["hacker_risk"]*(-dfi["amount"])
    # dfi["emp2"] =  dfi["trust"] * (dfi["amount"] ) + (1 - dfi["trust"])*(-dfi["amount"])
    # dfi["emp.risk.win"] = dfi["trust"] * (1 - dfi["hacker_risk"]) * (dfi["amount"] + dfi["win"]) + (1 - dfi["trust"] * (1 - dfi["hacker_risk"]))*(-dfi["amount"])
